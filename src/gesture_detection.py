...
def detect_gesture(image):
    """Detect hand gesture using MediaPipe Hands and return extra processed images."""
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    ...
    with mp_hands.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.7) as hands:
        ...
        if not results.multi_hand_landmarks:
            return "No hand detected", None, image, gray, thresh
        ...
        if up_count <= 1:
            return "Rock", hand_landmarks, image, gray, thresh
        elif fingers_up[1] and fingers_up[2] and not fingers_up[3] and not fingers_up[4]:
            return "Scissors", hand_landmarks, image, gray, thresh
        elif up_count == 5:
            return "Paper", hand_landmarks, image, gray, thresh
        elif fingers_up[3] and fingers_up[4] and not fingers_up[1] and not fingers_up[2]:
            return "Lizard", hand_landmarks, image, gray, thresh
        elif fingers_up[1] and fingers_up[2] and fingers_up[3] and not fingers_up[4]:
            return "Spock", hand_landmarks, image, gray, thresh
        else:
            return "Unknown", hand_landmarks, image, gray, thresh

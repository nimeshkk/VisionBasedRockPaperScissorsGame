...
def main():
    ...
    while True:
        ...
        gesture, landmarks, original, gray, thresh = detect_gesture(frame)
        ...
        if game_state == "detecting" and gesture in VALID_GESTURES:
            ...
            if current_time - last_gesture_time > gesture_cooldown:
                ...
                if gesture in VALID_GESTURES:
                    ...
                    print(f"Ignored invalid gesture: {gesture}")

        if game_state == "result":
            # Resize all images to 300px width and stack horizontally
            width = 300
            original = cv2.resize(original, (width, int(original.shape[0] * width / original.shape[1])))
            gray = cv2.cvtColor(cv2.resize(gray, (width, original.shape[0])), cv2.COLOR_GRAY2BGR)
            thresh = cv2.cvtColor(cv2.resize(thresh, (width, original.shape[0])), cv2.COLOR_GRAY2BGR)

            result_strip = cv2.hconcat([original, gray, thresh])

            # Add overlays on top
            result_text = f"You: {user_choice} {EMOJIS.get(user_choice)} | Computer: {computer_choice} {EMOJIS.get(computer_choice)} | {result} {REACTIONS.get(result)}"
            result_strip = add_text_overlay(result_strip, result_text, (10, 10), font_size=35, overlay_alpha=0.4)

            cv2.imshow("Game Result", result_strip)
        else:
            cv2.imshow("Rock Paper Scissors Lizard Spock", frame)
        ...
        if key == 27:  # ESC
            break
        elif game_state == "result" and key == 32:  # SPACE
            game_state = "detecting"
            user_choice = None
            computer_choice = None
            result = None
            cv2.destroyWindow("Game Result")
